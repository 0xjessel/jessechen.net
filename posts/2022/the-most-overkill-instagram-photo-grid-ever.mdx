...
![but why](/images/posts/2022/but-why.mp4)
...
_It costs 252kB to download one image_

Considering the Instagram photo grid I want is a 3x3 grid of thumbnails, showing 9 images would require us to download over 2MB just for thumbnails! To put this in context, the rest of my homepage resources add up to just 490kB.
...
This sounds awesome and exactly what I need, but how do I leverage this with remote images served from Graph API? The solution is to add a hardcoded allowlist of domains in your `next.config.js` file.
...
<aside>
❗ This is one of the drawbacks with my implementation.  A daily script to fetch new photos mean the photo grid can be up to 24 hours out of date.  This is an acceptable tradeoff for me, just follow me on Instagram if you really need to see my latest photo!
...
<aside>
  ⚠️ Make sure to also have a cron job to hit the [Refresh Access
  Token](https://developers.facebook.com/docs/instagram-basic-display-api/reference/refresh_access_token)
  endpoint every 60 days or so — this is to make sure your access token doesn’t
  expire.
</aside>
...
![video demo](/images/posts/2022/demo.mp4)
